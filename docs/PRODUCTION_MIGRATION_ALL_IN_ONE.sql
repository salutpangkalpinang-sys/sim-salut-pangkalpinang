...
CREATE TABLE IF NOT EXISTS public.students (
    ...
    entry_year INTEGER NULL CHECK ((entry_year >= 1980 AND entry_year <= 2100) OR (entry_year >= 19801 AND entry_year <= 21002)),
    faculty_id UUID REFERENCES public.faculties(id) ON DELETE RESTRICT,
    study_level_id UUID REFERENCES public.study_levels(id) ON DELETE RESTRICT,
    study_program_id UUID REFERENCES public.study_programs(id) ON DELETE RESTRICT,
    service_scheme_id UUID REFERENCES public.service_schemes(id) ON DELETE RESTRICT,
...
-- Function: verify_lip_document SECURITY DEFINER RPC
...
GRANT EXECUTE ON FUNCTION public.verify_lip_document(UUID, UUID) TO authenticated, service_role, anon;

-- Update Cash Account RPC (SECURITY DEFINER to bypass RLS)
