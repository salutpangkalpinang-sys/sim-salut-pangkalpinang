...
async function run() {
  ...
  // 1. Ensure SV (Sekolah Vokasi) faculty exists
  console.log("Upserting SV (Sekolah Vokasi) faculty...");
  const { data: facData, error: facErr } = await supabase
    .from("faculties")
    .upsert(
      [
        { code: "FKIP", name: "Fakultas Keguruan dan Ilmu Pendidikan" },
        { code: "FE", name: "Fakultas Ekonomi dan Bisnis" },
        { code: "FHISIP", name: "Fakultas Hukum, Ilmu Sosial dan Ilmu Politik" },
        { code: "FST", name: "Fakultas Sains dan Teknologi" },
        { code: "SV", name: "Sekolah Vokasi" },
      ],
      { onConflict: "code" }
    )
    .select();
  ...
  if (facErr) {
    console.error("Faculties upsert error:", facErr.message);
    process.exit(1);
  }

  console.log("Faculties upsert SUCCESS! Count:", facData.length);
  ...
  const feId = facMap["FE"] || facMap["FEB"];
  const fstId = facMap["FST"];
  const fhisipId = facMap["FHISIP"];
  const fkipId = facMap["FKIP"];
  const svId = facMap["SV"];

  const s1Id = levMap["S1"];
  const d4Id = levMap["D4"];
  const d3Id = levMap["D3"];
  ...
  const studyProgramsToUpsert = [
    ...
    { code: "411", name: "D-IV Kearsipan", faculty_id: svId, study_level_id: d4Id },
    { code: "412", name: "D-III Perpajakan", faculty_id: svId, study_level_id: d3Id },
  ];
