...
import { requestOperationalVoidAction, reviewOperationalVoidAction } from "@/features/operational/actions";
import { RoleCode } from "@/lib/auth/types";
import { X, Ban, AlertCircle, CheckCircle2, XCircle } from "lucide-react";

interface OperationalVoidDialogProps {
...
export function OperationalVoidDialog({
  transaction,
  isOpen,
  onClose,
  onSuccess,
  userRole,
}: OperationalVoidDialogProps) {
  ...
  const hasPendingVoidReq = transaction.voidRequest && transaction.voidRequest.status === "pending";
  ...
  const handleRequestVoid = async (e: React.FormEvent) => {
    ...
    try {
      const res = await requestOperationalVoidAction({
  ...
  const handleReviewVoid = async (action: "approve" | "reject") => {
    if (!transaction.voidRequest) return;
    setErrorMsg(null);
    ...
    setIsSubmitting(true);
    try {
      const res = await reviewOperationalVoidAction({
        voidRequestId: transaction.voidRequest.id,
        action,
      ...
      <div className="bg-white border border-slate-200 rounded-xl w-full max-w-md shadow-2xl overflow-hidden text-xs text-slate-900">
          ...
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-bold text-slate-900">
              {hasPendingVoidReq && isOwner
                ? "Persetujuan Void Transaksi Operasional (Owner)"
                : "Pengajuan Void Pembatalan Transaksi Operasional"}
            </h3>
          </div>
          <button
            type="button"
        ...
        <div className="p-6 space-y-4">
          <div className="space-y-1 bg-slate-50 p-3 rounded-lg border border-slate-200">
            <span className="text-[11px] text-slate-500 block">Transaksi Operasional</span>
            <p className="font-mono font-bold text-blue-600">{transaction.transactionNumber}</p>
          ...
          {hasPendingVoidReq && (
            <div className="bg-purple-50 border border-purple-200 p-3 rounded-lg text-purple-800 space-y-1">
              ...
              <p className="italic">&ldquo;{transaction.voidRequest?.reason}&rdquo;</p>
            </div>
          )}
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Jelaskan alasan pembatalan transaksi operasional..."
