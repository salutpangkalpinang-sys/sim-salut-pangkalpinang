...
import { toggleAcademicPeriodActiveAction, createAcademicPeriodAction } from "@/features/master-data/actions";

import { RoleCode, hasPermission } from "@/lib/auth/types";
import { Calendar, GraduationCap, Layers, CheckCircle2, Plus, DollarSign, UserCheck } from "lucide-react";
...
export function MasterDataContainer({ data, userRole }: MasterDataContainerProps) {
  const [activeTab, setActiveTab] = useState<"periods" | "programs" | "schemes" | "statuses" | "rates">("periods");
  ...
  const canManage = hasPermission(userRole, ["owner", "academic_admin"]);
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4 text-slate-900">
        <div>
          <h1 className="text-xl font-bold text-slate-900 tracking-tight">
            Master Data Akademik & Tarif
          </h1>
          <p className="text-xs text-slate-500 mt-1">
            Pengelolaan periode akademik, program studi, skema layanan, status mahasiswa, dan master tarif
          </p>
        </div>
      </div>
