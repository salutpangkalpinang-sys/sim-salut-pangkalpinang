...
export function VoidRequestDialog({
  ...
  const hasPendingVoidReq = payment.voidRequest && payment.voidRequest.status === "pending";
  ...
  const handleReviewVoid = async (action: "approve" | "reject") => {
    ...
    try {
      const res = await reviewPaymentVoidAction({
        voidRequestId: payment.voidRequest.id,
        action,
      ...
      <div className="bg-white border border-slate-200 rounded-xl w-full max-w-md shadow-2xl overflow-hidden text-xs text-slate-900">
        <div className="px-6 py-4 bg-slate-50 border-b border-slate-200 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-bold text-slate-900">
              {hasPendingVoidReq && isOwner ? "Persetujuan Void Pembayaran (Owner)" : "Pengajuan Void Pembatalan Transaksi"}
            </h3>
          </div>
          <button
            type="button"
        ...
        <div className="p-6 space-y-4">
          <div className="space-y-1 bg-slate-50 p-3 rounded-lg border border-slate-200">
            <span className="text-[11px] text-slate-500 block">Transaksi Pembayaran</span>
            <p className="font-mono font-bold text-blue-600">{payment.transactionNumber}</p>
            <p className="font-medium text-slate-900">{payment.studentName}</p>
            <p className="font-mono font-bold text-emerald-600">
              Rp {payment.amount.toLocaleString("id-ID")} ({payment.paymentMethodName})
            </p>
          </div>

          {hasPendingVoidReq && (
            <div className="bg-purple-50 border border-purple-200 p-3 rounded-lg text-purple-800 space-y-1">
              <span className="font-bold block">Alasan Pengajuan Void Kasir:</span>
              <p className="italic">&ldquo;{payment.voidRequest?.reason}&rdquo;</p>
            </div>
          )}
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Jelaskan alasan pembatalan transaksi pembayaran..."
